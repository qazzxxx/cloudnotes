...
import { FileAddOutlined, LogoutOutlined, SearchOutlined } from '@ant-design/icons';
...
/** 左侧栏：品牌区 + 搜索/新建 + 文件树 + 底部工具区（主题切换 / 退出）。 */
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="flex items-center gap-2 px-3 pb-2">
        ...
        />
        <Tooltip title="新建笔记">
          <Button icon={<FileAddOutlined />} onClick={() => setCreating({ parent: null, type: 'file' })} />
        </Tooltip>
      </div>

      {/* 文件树 / 搜索结果 */}
      <div className="flex-1 overflow-y-auto px-1.5 pb-3">
        <FileTree query={query} creating={creating} setCreating={setCreating} onNavigate={onNavigate} />
      </div>
